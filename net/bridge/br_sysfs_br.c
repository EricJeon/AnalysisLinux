...
static ssize_t show_br_drop_disabled(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(br_drop_disabled, S_IRUGO, show_br_drop_disabled,
		   NULL);

static ssize_t show_br_drop_loop(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(br_drop_loop, S_IRUGO, show_br_drop_loop,
		   NULL);

static ssize_t show_br_drop_learning(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(br_drop_learning, S_IRUGO, show_br_drop_learning,
		   NULL);

static ssize_t show_br_drop_pause(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(br_drop_pause, S_IRUGO, show_br_drop_pause,
		   NULL);

static ssize_t show_br_drop_bad_ip_options(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%ld\n", br->br_drop_bad_ip_options);
}
static DEVICE_ATTR(br_drop_bad_ip_options, S_IRUGO, show_br_drop_bad_ip_options,
		   NULL);

static ssize_t show_br_drop_over_mtu(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(br_drop_over_mtu, S_IRUGO, show_br_drop_over_mtu,
		   NULL);

static ssize_t show_br_drop_frag_copy_err(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(br_drop_frag_copy_err, S_IRUGO, show_br_drop_frag_copy_err,
		   NULL);

static ssize_t show_br_drop_fail_ip_route_in(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%ld\n", br->br_drop_fail_ip_route_in);
}
static DEVICE_ATTR(br_drop_fail_ip_route_in, S_IRUGO, show_br_drop_fail_ip_route_in,
		   NULL);

static ssize_t show_br_drop_fail_ip_route_out(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%ld\n", br->br_drop_fail_ip_route_out);
}
static DEVICE_ATTR(br_drop_fail_ip_route_out, S_IRUGO, show_br_drop_fail_ip_route_out,
		   NULL);

static ssize_t show_br_drop_no_parent_dev(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%ld\n", br->br_drop_no_parent_dev);
}
static DEVICE_ATTR(br_drop_no_parent_dev, S_IRUGO, show_br_drop_no_parent_dev,
		   NULL);

static ssize_t show_br_drop_no_unshare(struct device *d,
					  struct device_attribute *attr,
					  char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%ld\n", br->br_drop_no_unshare);
}
static DEVICE_ATTR(br_drop_no_unshare, S_IRUGO, show_br_drop_no_unshare,
		   NULL);
...
static struct attribute *bridge_attrs[] = {
	...
	&dev_attr_flush.attr,
	&dev_attr_br_drop_disabled.attr,
	&dev_attr_br_drop_loop.attr,
	&dev_attr_br_drop_learning.attr,
	&dev_attr_br_drop_pause.attr,
	&dev_attr_br_drop_bad_ip_options.attr,
	&dev_attr_br_drop_over_mtu.attr,
	&dev_attr_br_drop_frag_copy_err.attr,
	&dev_attr_br_drop_fail_ip_route_in.attr,
	&dev_attr_br_drop_fail_ip_route_out.attr,
	&dev_attr_br_drop_no_parent_dev.attr,
	&dev_attr_br_drop_no_unshare.attr,
#ifdef CONFIG_BRIDGE_IGMP_SNOOPING
