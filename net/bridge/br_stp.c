...
void br_become_designated_port(struct net_bridge_port *p)
{
 ...
 /* called under bridge lock */
void br_gen_updown_event(struct net_bridge *br)
{
	struct net_bridge_port *p;
	int flag = 0;

	//br_info(br, "br_gen_updown_event - evaluate carrier state\n");
	...
	if (flag == 1) {
		if (netif_running(br->dev) && !netif_carrier_ok(br->dev)) {
			br_info(br, "br_gen_updown_event -  sending carrier on to %s\n", br->dev->name);
			netif_carrier_on(br->dev);
		}
	} else {
		if (netif_running(br->dev) && netif_carrier_ok(br->dev)) {
			br_info(br, "br_gen_updown_event - sending carrier off to %s\n", br->dev->name);
