...
#define MAX_MEMPARTNAME_LEN	16

struct bmem_region {
#if defined(CONFIG_DTV_RESERVED_MEM)
	char			name[MAX_MEMPARTNAME_LEN];
#endif
	unsigned long	addr;
	unsigned long	size;
	int				valid;
...
static int __init parse_resmem_regions(char *str)
{
	unsigned long addr = 0, size;
	unsigned long lower_mem_bytes;
	char *orig_str = str;
	...
	if (*str == '@') {
		...
		printk(KERN_ERR "resmem: addr not specified\n");
		return 0;
	}
	...
	if (*str == '(') {
		...
		q = strchr(str, ')');
		if (q) {
			name_len = ((q-str) < (MAX_MEMPARTNAME_LEN-1)) ?(q-str) : (MAX_MEMPARTNAME_LEN-1);
		...
		printk(KERN_ERR "resmem: name not specified\n");
		return 0;
	}

	if ((size > 0x40000000) || (addr > 0x80000000)) {
		printk(KERN_WARNING "resmem: argument '%s' "
			"is out of range, ignoring\n", orig_str);
		return 0;
	}

	if (size == 0) {
		printk(KERN_INFO "resmem: disabling reserved memory\n");
		bmem_disabled = 1;
		return 0;
	}

	if ((addr & ~PAGE_MASK) || (size & ~PAGE_MASK)) {
		printk(KERN_WARNING "resmem: ignoring invalid range '%s' "
			"(is it missing an 'M' suffix?)\n", orig_str);
		return 0;
	}
	...
	strncpy(bmem_regions[n_bmem_regions].name, name, MAX_MEMPARTNAME_LEN);
	bmem_regions[n_bmem_regions].addr = addr;
	bmem_regions[n_bmem_regions].size = size;
	n_bmem_regions++;
...
early_param("resmem", parse_resmem_regions);
...
/*
 * Frees the region that starts at addr and has the size specified, if it exists.
 */
int bmem_free_region(unsigned long addr, unsigned long size)
{
	int i = 0;

	// Find the bmem region that matches
	for (i = 0; i < n_bmem_regions; i++) {
	  if (!bmem_regions[i].valid)
	    continue;
	  if ((addr == bmem_regions[i].addr) && (size == bmem_regions[i].size)) {
	      free_init_pages("bslrfs", addr, addr+size);
	      // We're done now, so get out.
	      return 0;
	  }
	}
	// Loop finished and no bmem region was found...
	return -ENOENT;
}
EXPORT_SYMBOL(bmem_free_region);
...
static int __init bmem_region_setup(void)
{
	...
	for (i = 0; i < n_bmem_regions; i++) {
		...
		name = kzalloc(MAX_MEMPARTNAME_LEN, GFP_KERNEL);
		if (!r || !name)
			break;

#if defined(CONFIG_DTV_RESERVED_MEM)
		if (*bmem_regions[i].name == '\0') {
			sprintf(name, "bmem.%d", idx);
			idx++;
		}
		else {
			strncpy(name, bmem_regions[i].name, MAX_MEMPARTNAME_LEN);
		}
#else
			sprintf(name, "bmem.%d", idx);
			idx++;
