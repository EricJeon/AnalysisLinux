...
u8 brcm_primary_macaddr[BRCM_MAC_TYPE_MAX][IFHWADDRLEN] = {{0x00, 0x00, 0xde, 0xad, 0xbe, 0xef}, {0x00, 0x00, 0xde, 0xad, 0xbf, 0xef},};
...
int brcm_alloc_macaddr(int type, u8 *buf)
{
	printk("dtv: reading MAC%d address...\n", type);
	memcpy(buf, brcm_primary_macaddr[type], ETH_ALEN);
	return 0;
}
