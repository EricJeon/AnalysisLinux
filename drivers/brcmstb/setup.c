...
void humax_reset_control_gpio_reset(void);
static unsigned int common_variable;
#define MASK_HMX(Register,Field) \
        BCHP_##Register##_##Field##_MASK

#define FIELD_DATA_HMX(Register,Field,Data) \
        ((Data) << SHIFT_HMX(Register,Field))

#define SHIFT_HMX(Register,Field) \
        BCHP_##Register##_##Field##_SHIFT

#define RESET_IODIR 0x00408c08
#define RESET_DATA 0x00408c04
#define RESET_GPIO 20
#define WDCTRL_ADDRESS  0x004067fc
#endif
...
static void __init brcm_register_genet(int id, struct bcmemac_platform_data *pd)
{
	...
	if (pd->phy_type == BRCM_PHY_TYPE_INT)
		brcm_alloc_macaddr(BRCM_MAC_TYPE_ETH, pd->macaddr);
	else if (pd->phy_type == BRCM_PHY_TYPE_MOCA)
		brcm_alloc_macaddr(BRCM_MAC_TYPE_MOCA, pd->macaddr);
...
static void __init brcm_register_moca(int enet_id)
{
	...
	brcm_alloc_macaddr(BRCM_MAC_TYPE_MOCA, macaddr);
	...
	struct mtd_partition *parts)
{
	...
	case TYPE_NOR: {
		...
		pdev = platform_device_alloc("dtvflash", nor_id++);
		if (!pdev ||
...
#if defined(CONFIG_HUMAX_PLATFORM)
	{
		volatile unsigned long ebi_control_reg;

		ebi_control_reg = BDEV_RD(BCHP_EBI_CS_CONFIG_0);
		ebi_control_reg &= ~BCHP_EBI_CS_CONFIG_0_mask_en_MASK;
		BDEV_WR(BCHP_EBI_CS_CONFIG_0, ebi_control_reg);
	}
...
#if defined(CONFIG_HUMAX_PLATFORM)
//for HR54 GPIO reset for kernel panic.
void humax_reset_control_gpio_reset(void)
{
        printk(KERN_EMERG"hum: triggering GPIO RESET !!!!!!\n");

        common_variable = *(volatile unsigned int*)(BVIRTADDR(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_2));
        common_variable &= ~(MASK_HMX(AON_PIN_CTRL_PIN_MUX_CTRL_2, aon_gpio_020 ));
        common_variable |= FIELD_DATA_HMX(AON_PIN_CTRL_PIN_MUX_CTRL_2, aon_gpio_020, 0 );
        *(volatile unsigned int*)(BVIRTADDR(BCHP_AON_PIN_CTRL_PIN_MUX_CTRL_2)) = common_variable;

        common_variable = *(volatile unsigned int*)(BVIRTADDR(RESET_IODIR));
        *(volatile unsigned int*)(BVIRTADDR(RESET_IODIR)) = (common_variable & ~(1<<RESET_GPIO));

        common_variable = *(volatile unsigned int*)(BVIRTADDR(RESET_DATA));
        *(volatile unsigned int*)(BVIRTADDR(RESET_DATA)) = (common_variable | (1<<RESET_GPIO));
...
{
#if defined(CONFIG_HUMAX_PLATFORM)
	humax_reset_control_gpio_reset();
#endif  /* CONFIG_HUMAX_PLATFORM */
