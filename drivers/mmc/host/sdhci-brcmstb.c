...
static int sdhci_brcmstb_config(struct platform_device *pdev)
{
	...
	SDIO_CFG_SET(cfg_base, CAP_REG1, CAP_REG_OVERRIDE);
	/*
	 * Use manual input clock delay to work around timing issues.
	 * Enable, resolution = 2, value = 15
	 */
	SDIO_CFG_FIELD(cfg_base, IP_DLY, IP_TAP_DELAY, 15);
	SDIO_CFG_FIELD(cfg_base, IP_DLY, IP_DELAY_CTRL, 2);
...
#elif defined(CONFIG_BCM7346B0)
	/*
	 * Leave the UHS bits alone because they get set when writing
	 * HOST_CONTROL.
	 */
	if (reg == SDHCI_HOST_CONTROL2) {
		u16 uhs;
		uhs = __raw_readw(host->ioaddr + SDHCI_HOST_CONTROL2);
		uhs &= SDHCI_CTRL_UHS_MASK;
		val &= ~SDHCI_CTRL_UHS_MASK;
		val |= uhs;
	}
#endif
...
static u8 sdhci_brcmstb_readb(struct sdhci_host *host, int reg)
{
	u8 val;

	/*
	 * The HISPD bit in CONTROL gets mapped to UHS_SDR50 in CONTROL2
	 * so reverse the mapping.
	 */
	val = __raw_readb(host->ioaddr + reg);
	if (reg == SDHCI_HOST_CONTROL) {
		u16 ctl2;

		ctl2 = __raw_readw(host->ioaddr + SDHCI_HOST_CONTROL2);
		if ((ctl2 & SDHCI_CTRL_UHS_MASK) == SDHCI_CTRL_UHS_SDR50)
...
{
#if defined(CONFIG_BCM7346B0)
	/*
	 * Map the HISPD bit in CONTROL to UHS_SDR50 in CONTROL2.
	 * This is needed to improve input and output timing (SWLINUX-2932)
	 */
	if (reg == SDHCI_HOST_CONTROL) {
		u16 ctl2;

		ctl2 = __raw_readw(host->ioaddr + SDHCI_HOST_CONTROL2);
		ctl2 &= ~SDHCI_CTRL_UHS_MASK;
		if (val & SDHCI_CTRL_HISPD) {
			ctl2 |= SDHCI_CTRL_UHS_SDR50;
		}
		__raw_writew(ctl2, host->ioaddr + SDHCI_HOST_CONTROL2);
		val &= ~SDHCI_CTRL_HISPD;
	}
#endif
