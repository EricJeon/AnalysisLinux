...
#define FETCH(name, fn, arg) do { \
	if (cfe_getenv(name, cfe_buf, COMMAND_LINE_SIZE) == CFE_OK) { \
       ...
       brcm_dram0_size_mb = CONFIG_BRCM_FORCED_DRAM0_SIZE;
       FETCH("BOOT_FLAGS", parse_cmdline, arcs_cmdline); /* BSS will only provide this as kernel parameter */

       /* don't need brcm_mtd_xxx_start / len if use fixed partition */
       brcm_mtd_flash_size_mb = CONFIG_BRCM_PLATFORM_FLASH_SIZE;
       parse_string("SPI", brcm_mtd_flash_type);
#if 0
...
static void __init read_mac_addresses(void)
{
	const volatile void *mac_addr;
	int type;
	
#if defined(CONFIG_BRCM_PLATFORM_MACADDR0_OFFSET)
#if defined(CONFIG_BRCM_MOCA_ON_GENET_0)
	type = BRCM_MAC_TYPE_MOCA;
#else
	type = BRCM_MAC_TYPE_ETH;
#endif

	mac_addr = (const volatile void *)(CONFIG_BRCM_PLATFORM_FLASH_START_ADDR + CONFIG_BRCM_PLATFORM_MACADDR0_OFFSET);
	memcpy_fromio(brcm_primary_macaddr[type], mac_addr, ETH_ALEN);
	printk("MAC%d address : %02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n", type,
		brcm_primary_macaddr[type][0],
		brcm_primary_macaddr[type][1],
		brcm_primary_macaddr[type][2],
		brcm_primary_macaddr[type][3],
		brcm_primary_macaddr[type][4],
		brcm_primary_macaddr[type][5]);
#endif

#if defined(CONFIG_BRCM_PLATFORM_MACADDR1_OFFSET)
	...
	mac_addr = (const volatile void *)(CONFIG_BRCM_PLATFORM_FLASH_START_ADDR + CONFIG_BRCM_PLATFORM_MACADDR1_OFFSET);
	memcpy_fromio(brcm_primary_macaddr[type], mac_addr, ETH_ALEN);
